...
import SourceCodePopup from './popup/SourceCodePopup';

const SourceCode = () => {
  const { sourceCode, updateSourceCode } = useModel('companydata');
  const intl = useIntl();
  ...
  const [recordUpdate, setRecordUpdate] = useState({ ma: 0, sourceCode: '' });

  const handleTableChange = (pagination, filters, sorter) => {
    setPagination({ ...pagination, total: sourceCode.length });
    updateSourceCode();
  };
  ...
  const showAddSourceCode = () => {
    setAddOrUpdate('add');
    handleShowPopup();
  };

  const showUpdateSourceCode = () => {
    setAddOrUpdate('update');
    handleShowPopup();
  };

  const deleteSourceCode = (idDelete) => {
    const formData = new FormData();
    formData.append('id', idDelete);
    useFetch(
      '/api/Company/DeleteSourceCode',
      'DELETE',
      '',
      formData,
      (res) => {
        if (res.success == 1) {
          ...
          updateSourceCode();
          setPagination({ ...pagination, total: sourceCode.length - 1 });
        } else {
  ...
  return (
    <Row className="SourceCode-container" gutter={[16, 16]}>
      <SourceCodePopup
      ...
      <Col
        ...
        xxl={{ span: 24 }}
      >
        <buttonList.add className="btn-add" onClick={showAddSourceCode} />
      </Col>
      <Col
        xs={{ span: 24 }}
        sm={{ span: 24 }}
            ...
            return {
              ...
              onClick: (event) => {
                if (event.target.nodeName == 'TD') {
                  setRecordUpdate({ ma: record.ma, sourceCode: record.sourceCode });
                  showUpdateSourceCode();
                }
              }, // click row
            };
          }}
