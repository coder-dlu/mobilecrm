...
const SelectMenu = ({ listSL, className }) => {
...
const selectMenu = {
  ...
  option: ({ value, children, onClick, size, isChildren }) => {
    if (isChildren) {
      return (
        <div
          style={{ fontSize: size === 'large' ? '16px' : '14px', paddingLeft: '10%' }}
      ...
      return (
        <div
          style={{ fontSize: size === 'large' ? '16px' : '14px', fontWeight: 'bold' }}
          onClick={() => onClick(value, children)}
        >
          {children}
        </div>
...
export { dropdownMenu, SelectMenu, selectMenu };
