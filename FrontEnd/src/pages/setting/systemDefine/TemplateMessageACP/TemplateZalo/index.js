...
import { Button, message, Input, Space, Table, Popconfirm, Modal, Radio } from 'antd';
...
function TemplateZalo({ method, language }) {
  //table
  const intl = useIntl();
  ...
  useEffect(() => {
    ...
    GetTemplateByChannelLanguage(data).then((res) => setDataZalo(res.data));
  }, []);
  ...
  const onDeleteTemplateZalo = async (e, record) => {
    console.log(record.id);
    e.stopPropagation(); // stop propagation to prevent triggering onClick for parent elements
    try {
      await deleteTemplate(record.id);
      setDataZalo(dataZalo.filter((teamPlate) => teamPlate.id !== record.id));
    } catch (error) {
      console.error(error);
    }
  };
  const dataList = dataZalo.map((item, i) => ({
    id: item.id,
    stt: i + 1,
    name: item.name,
    ...
    // },
    {
      ...
      render: (e, record) => (
        <Popconfirm
          ...
          onConfirm={(e) => onDeleteTemplateZalo(e, record)}
          okText="Có"
          cancelText="Không"
        >
  ...
  return (
    <>
      ...
      <h2 className="titleTemplate">
        {intl.formatMessage({ id: 'pages.setting.AddTemplateACP.modal.title' })}
      </h2>
