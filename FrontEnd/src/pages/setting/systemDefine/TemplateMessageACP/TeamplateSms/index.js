...
function TemplateSms({ method, language }) {
  //table
  const intl = useIntl();
  ...
  useEffect(() => {
    ...
    GetTemplateByChannelLanguage(data).then((res) => setDataSms(res.data));
  }, []);
  const reloadData = () => {
    const data = {
      channel: method,
  ...
  const onDeleteTemplateSms = async (e, record) => {
    console.log(record.id);
    e.stopPropagation(); // stop propagation to prevent triggering onClick for parent elements
    try {
      await deleteTemplate(record.id);
      setDataSms(dataSms.filter((teamPlate) => teamPlate.id !== record.id));
    } catch (error) {
      console.error(error);
    }
  };
  const dataList = dataSms.map((item, i) => ({
    id: item.id,
    stt: i + 1,
    name: item.name,
    ...
    // },
    {
      ...
      render: (e, record) => (
        <Popconfirm
          ...
          onConfirm={(e) => onDeleteTemplateSms(e, record)}
          okText="Có"
          cancelText="Không"
        >
